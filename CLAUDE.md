...
| 事業 | 状態 | 成果物の置き場所 |
|---|---|---|
| Javabronze(Java Bronze クイズアプリ) | 開発中 | `docs/<部署>/` |
| レンタルスペース事業 | 立ち上げ準備 | `docs/rental-space/<部署>/` |

AI社員の定義は Javabronze 向けに書かれているが、別事業の依頼では題材だけ読み替え、各自のルール(出典明記・担当外は渡す・外部公開は社長確認など)はそのまま守る。
